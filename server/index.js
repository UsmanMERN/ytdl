...
app.get('/api/video', async (req, res) => {
    try {
        ...
        const videoWithAudioFormats = videoInfo.formats
            .filter(format => format.hasVideo && format.hasAudio)
            .sort((a, b) => (b.height || 0) - (a.height || 0));
        ...
        const data = {
            ...
            embed_url: `https://www.youtube.com/embed/${videoInfo.videoDetails.videoId}`,
            best_audio: audioFormats[0] || null,
            best_video: videoWithAudioFormats[0] || null,
            all_formats: videoInfo.formats
        };
        ...
        if (error.message.includes('This video is unavailable')) {
            ...
            return res.status(403).send({ error: 'This video is age-restricted. Authentication required.' });
        }
        return res.status(500).send({ error: 'Failed to get video information' });
    }
});
...
app.get('/api/download', (req, res) => {
    const url = req.query.url;
    const itag = req.query.format; // assuming format is the itag
    const title = req.query.title;
    const ext = req.query.ext;

    if (!url || !itag) {
        return res.status(400).send({ error: 'URL and format (itag) are required' });
    }
